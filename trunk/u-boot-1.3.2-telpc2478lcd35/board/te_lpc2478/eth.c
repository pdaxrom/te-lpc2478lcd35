...
static int 
check_Micrel_KSZ8721BL(void)
{
  volatile u32 regValue = 0;

	volatile u32 phyID1, phyID2;
	
	phyID1 = readPhy(PHY_PHYIDR1);
	phyID2 = readPhy(PHY_PHYIDR2);
	
	
	printk("\r\nemac: check_phy - (%x, %x)\r\n", phyID1, phyID2);
	
	//if ( 	(	(phyID1&0xffff) != 0x0022) ||
	//			(	(phyID2&0xfff0) != 0x1619) ) return FALSE;
...
// TODO do we really need to set this again?
  ...
  else
  {
	...
	phyAddr = 0x100;
	if (check_Micrel_KSZ8721BL() == FALSE)
	{
		printk("emac: cannot identify external PHY\r\n");
      phyAddr = 0;
      printk(": [0]->(%x, %x)\r\n", readPhy(PHY_PHYIDR1), readPhy(PHY_PHYIDR2));
      phyAddr = 0x100;
      printk(": [1]->(%x, %x)\r\n", readPhy(PHY_PHYIDR1), readPhy(PHY_PHYIDR2));
      return -1;
	}
	phyType = MICREL_PHY;
	printk("emac: found Micrel KSZ8721BL PHY");
