...
int lpc2468_copy_buffer_to_flash(flash_info_t * info, ulong flash_addr)
{
  int first_sector;
  int last_sector;
...
int lpc2468_flash_erase_philips (flash_info_t * info, int s_first, int s_last)
{
  int flag;
  int prot;
  int sect;
...
int lpc2468_write_buff_philips (flash_info_t * info, 
			uchar * src, 
			ulong addr, 
			ulong cnt)
{
  ...
  memcpy((void*)(COPY_BUFFER_LOCATION + 512 - first_copy_size), 
	 src, 
	 first_copy_size);
  lpc2468_copy_buffer_to_flash(info, first_block * 512);
  src += first_copy_size;
  ...
  for (i = 0; i < nbr_mid_blocks; i++) {
    ...
    memcpy((void*)COPY_BUFFER_LOCATION, 
	   src, 
	   512);
    lpc2468_copy_buffer_to_flash(info, addr);
    src += 512;
    addr += 512;    
  }
